<?php  
	...
	if (isset($_POST['hazard'])) {
		...
		if (mysqli_query($conn,$acc) === TRUE) {
			...
			$hazard = "INSERT INTO tbl_report(type_of_report,Incident_type,brgy,landmark,description,date_report,status,account_type,report_account_fk) VALUES('$type',null,'$brgy','$landmark','$des',NOW(),'pending','User',$last_id)";
		...
		if (mysqli_query($conn,$acc_inci) === TRUE) {
			$last_id = mysqli_insert_id($conn);
			$Incident_sql = "INSERT INTO tbl_report(type_of_report,Incident_type,brgy,landmark,description,date_report,status,account_type,report_account_fk) VALUES('$type','$Incident_type','$brgy','$land','$des',NOW(),'pending','User',$last_id)";
